...
أوّل أمر منهما يُنزّل أرشيف المصدر (91 MB) من
[قرآنبيديا](https://api.quranpedia.net) إن لم يكن موجودًا، ثم يُعاد استعماله.
...
| `OWNER_IDS` | — | *(فارغ)* | معرّفات تيليجرام لمن يفتح لوحة المالك `/owner`، مفصولة بفواصل. بلا قيمة لا تعمل اللوحة لأحد |
...
## لوحة المالك (اختيارية)

لمن يشغّل البوت لوحة إحصاءات عن كل مجموعاته مجتمعة: عدد المجموعات النشطة
والمتوقّفة، عدد الأشخاص والاشتراكات، الختمات المكتملة والجارية، الصفحات المقروءة
عبر المجموعات كلها، ونشاط اليوم والأسبوع.

```bash
# في .env — معرّفك الرقمي من @userinfobot
OWNER_IDS=123456789
```

ثم أرسل `/owner` في محادثة **خاصة** مع البوت. اللوحة:

- لا تعمل داخل أي مجموعة، وليست في قائمة الأوامر.
- لا تجيب أحدًا إن كان `OWNER_IDS` فارغًا — وهو الافتراضي.
- تتحقّق من المالك عند كل ضغطة زر لا عند فتحها فقط، فرسالة مُعاد توجيهها لا تحمل
  أزرارًا عاملة.
- من غير المالك يُرسل `/owner` فلا يتلقّى شيئًا — لا وجود للّوحة من طرفه.
...
scripts/source_archive.py   تنزيل أرشيف المصدر عند الحاجة
...
صور المصحف ونصّه من رواية حفص مصدرها موقع **[قرآنبيديا — Quranpedia](https://api.quranpedia.net)**،
جزى الله القائمين عليه خيرًا. الأرشيف
[`hafs.zip`](https://quranpedia.net/api-quran-svg/hafs.zip) فيه 604 صفحة SVG،
وملف مواضع الآيات في كل صفحة، و `mushaf-content.json` بنصّ المصحف كاملًا مع
الجزء والحزب ورقم الصفحة لكل آية.

الأرشيف غير مرفوع مع المستودع (91 MB)، وكذلك الصور المبنيّة منه، **ويُنزَّل
تلقائيًا** عند أول تشغيل لأيٍّ من سكربتَي البناء ويُحفظ في `assets/hafs.zip`
فلا يُنزَّل مرّة أخرى. ومن كانت عنده نسخة فليضعها هناك مسبقًا، ومن أراد منع
التنزيل فليمرّر `--no-download`.
...
شيفرة المشروع تحت رخصة **MIT** — انظر [`LICENSE`](LICENSE). استعملها وعدّلها
وانشرها كما تشاء.

أمّا نصّ القرآن وصور المصحف فليست جزءًا من هذه الرخصة: مصدرها
[قرآنبيديا](https://api.quranpedia.net)، وتُنزَّل وقت البناء ولا تُوزَّع مع
الشيفرة، ولها أحكامها وحرمتها.
